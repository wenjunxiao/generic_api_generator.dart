...
Builder genericApiBuilder([BuilderOptions options]) {
  bool autoCastResponse =
      (options?.config['auto_cast_response']?.toString() ?? 'true') == 'true';
  return SharedPartBuilder(
    [
      GenericApiGenerator(autoCastResponse: autoCastResponse),
    ],
    'generic_api',
  );
}

class GenericApiGenerator extends GeneratorForAnnotation<retrofit.RestApi> {
  ...
  var autoCastResponse = true;

  GenericApiGenerator({this.autoCastResponse});
  ...
  String _implementClass(ClassElement element, ConstantReader annotation) {
    ...
    return '${classBuilder.accept(emitter)}';
  }
  ...
  Code _generateRequest(MethodElement m, ConstantReader httpMehod) {
    ...
    autoCastResponse = (autoCastResponse ??
        ...
        if (_isBasicType(innerReturnType)) {
          ...
          switch (clientAnnotation.parser) {
            case retrofit.Parser.JsonSerializable:
              if (innerReturnType.element is ClassElement &&
                  innerReturnType is InterfaceType) {
                ClassElement element = innerReturnType.element;
                var ps = [];
                for (var i = 0; i < element.typeParameters.length; i++) {
                  final a = innerReturnType.typeArguments[i];
                  if (_isBasicType(a)) {
                    ps.add(
                        'fromJson${i + 1}: (v) => v as ${a.getDisplayString()}');
                  } else {
                    ps.add(
                        'fromJson${i + 1}: (v) => ${a.getDisplayString()}.fromJson(v)');
                  }
                }
                blocks.add(Code(
                    "var value = $_resultVar.data.map((dynamic i) => $innerReturnType.fromJson(i as Map<String,dynamic>, ${ps.join(',')})).toList();"));
              } else {
                blocks.add(Code(
          ...
          switch (clientAnnotation.parser) {
            case retrofit.Parser.JsonSerializable:
              if (returnType.element is ClassElement &&
                ...
                var ps = [];
                for (var i = 0; i < element.typeParameters.length; i++) {
                  final a = returnType.typeArguments[i];
                  if (_isBasicType(a)) {
                    ps.add(
                        'fromJson${i + 1}: (v) => v as ${a.getDisplayString()}');
                  } else {
                ...
                blocks.add(Code(
                    "final value = $returnType.fromJson($_resultVar.data, ${ps.join(',')});"));
              } else {
                blocks.add(Code(
  ...
  void _generateQueries(
    ...
    for (final p in queryMap.keys) {
      ...
      if (type is InterfaceType) {
        for (var i = 0; i < type.element.typeParameters.length; i++) {
          final a = type.typeArguments[i];
          if (_isBasicType(a)) {
            ps.add('toJson${i + 1}: (v) => v');
          } else {
            ps.add('toJson${i + 1}: (${a.getDisplayString()} v) => v.toJson()');
          }
        }
      }
      final value =
    ...
    if (_bodyName != null) {
      if (TypeChecker.fromRuntime(Map).isAssignableFromType(_bodyName.type)) {
        ...
        if (toJson == null) {
          ...
          if (_bodyName.type is InterfaceType) {
            final bodyType = _bodyName.type as InterfaceType;
            var ps = [];
            for (var i = 0; i < ele.typeParameters.length; i++) {
              final a = bodyType.typeArguments[i];
              if (_isBasicType(a)) {
                ps.add('toJson${i + 1}: (v) => v');
              } else {
                ps.add(
                    'toJson${i + 1}: (${a.getDisplayString()} v) => v.toJson()');
              }
            }
            blocks.add(refer(
                    "${_bodyName.displayName}?.toJson(${ps.join(',')}) ?? <String,dynamic>{}")
                .assignFinal(_dataVar)
                .statement);
          } else {
            blocks.add(refer(
